...
const buildSnapMap = (basket: BasketType): BasketSnapMap => {
  ...
  const isSame = (a: number, b: number) => Math.abs(a - b) < 1e-6;

  const isCorner = (side: "x" | "z", snap: SnapPoint) => {
    if (!excludeCorners) return false;
    if (side === "z") return isSame(snap.x, minXSnap) || isSame(snap.x, maxXSnap);
    return isSame(snap.z, minZSnap) || isSame(snap.z, maxZSnap);
  };
